...
bool rearrangeStringToPalindrome(char* str) {
    int Hist[ALPHBT] = {0};
    int length = 0; //use as both running index and get length
    while (str[length]) {
        Hist[str[length]-'a']++;
        length++;
    }
    int letter_odd = -1;
    //testing histogram:
    for (int letter = 0 ; letter < ALPHBT ; letter++) {
        if (Hist[letter] % 2 ) //odd count letter:
        {
            if (letter_odd == -1) {
                letter_odd = letter;
            }
            else {
                //cant have two odd count letters:
                return false;
            }
        }
    }
    if (letter_odd != -1) {
        str[length/2] = (char)('a' + letter_odd);
        Hist[letter_odd]--;
    }
    int offset = 0;
    for (int letter = 0 ; letter < ALPHBT ; letter++) {
        while(Hist[letter]) {
            str[offset] = (char)('a' + letter);
            str[length - 1 - offset] = (char)('a' + letter);
            Hist[letter] -= 2;
            offset++;
        }
    }
