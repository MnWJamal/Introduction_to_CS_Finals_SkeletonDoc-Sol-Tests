...
bool read_names(char*** names, int* n) {
    ...
    *names = malloc(*n * sizeof(char*));
    if (!*names) return false; //!*names and not !names
    //MOVED: name alloc to for on (**)
    for (int i = 0; i < *n; i++) (*names)[i] = NULL;

    for (int i = 0; i < *n; i++) {
        //(**) =>
        char* name = malloc(MAX_LEN * sizeof(char)); //added *sizeof(char)
        if (!name) //added malloc check and free previous successful
        {
            free_names((*names), *n);
            return false;
        }
