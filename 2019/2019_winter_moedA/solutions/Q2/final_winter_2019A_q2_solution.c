...
void ZigzagSymetricArray(int arr[], int n) {
    //take an array that is half negative and half positive and "zigzags" it
    for (int ziggy = 1 ; ziggy < n/2 ; ziggy += 2) {
        Swap(arr+ziggy, arr+n-ziggy-1);
    }
}
void AlternatingArrange(int arr[], int n) {
    // first we push positives to right:
    int put_positive = n-1;
    for (int i = 0; i <= put_positive ; i++) //O(n)
    {
        if (arr[i] >= 0) {
            Swap(arr+i, arr+put_positive);
            i--;
            put_positive--;
        }
    }
    //now put_positive is last NEGATIVE index, so:
    int negative_ct = put_positive+1;
    int positive_ct = n - negative_ct;
    if (negative_ct > positive_ct) {
        int delta = negative_ct - positive_ct;
        ZigzagSymetricArray(arr+delta,n-delta);
    }
    else {
        int delta = positive_ct - negative_ct;
        ZigzagSymetricArray(arr,n-delta);
    }
