...
bool Proportionalable_aux(int arr[], int n, int number) {
    if (number == n+1) return true;
    for(int i = 0 ; i < 2*n ; i++)
    {
        if (arr[i] == 0 && i+number+1 < 2*n && arr[ i+number+1] == 0) {
            //try here:
            arr[i] = number;
            arr[i+number+1] = number;
            if (Proportionalable_aux(arr,n,number+1)) return true; //doesnt backtrack
            //backtrack on failiure:
            arr[i] = 0;
            arr[i+number+1] = 0;
        }

    }
    //no fit for fumber => bad:
    return false;
}
bool Proportionalable(int arr[], int n) {
    for (int i = 0 ; i < n ; i++) arr[i] = 0;
    return Proportionalable_aux(arr,n,1);
}
